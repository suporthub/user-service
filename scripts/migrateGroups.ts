...
async function main() {
  ...
  console.log('🗑️  Erasing existing group data...');

  // We use a transaction to ensure the cleanup is atomic
  await prisma.$transaction([
    prisma.groupSymbol.deleteMany(), // Delete dependencies first
    prisma.group.deleteMany(),
  ]);

  console.log(`🚀 Migrating ${dataLines.length} groups...`);
  ...
  for (const line of dataLines) {
    ...
    console.log(`✅ Created group: ${name} (UUID: ${group.id})`);
  }
